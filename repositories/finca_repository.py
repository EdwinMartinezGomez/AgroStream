...
class FincaRepository:
    """Persistencia de fincas en Redis."""
    ...
    def eliminar(self, finca_id: str) -> bool:
        ...
        # Limpieza de lecturas y alertas asociadas a la finca.
        if deleted:
    ...
    def _migrar_desde_lecturas_legacy(self) -> None:
        """
        Compatibilidad: si no existen metadatos CRUD, intenta reconstruir
        fincas a partir de `finca:{id}:ultima` que ya contiene lecturas JSON.
        """
        migradas = 0
        ahora = datetime.now(tz=timezone.utc).isoformat()
